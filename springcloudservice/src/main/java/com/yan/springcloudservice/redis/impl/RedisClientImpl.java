...
public class RedisClientImpl extends DefaultClientImpl implements IRedisClient {
    ...
    Logger logger = LoggerFactory.getLogger(RedisClientImpl.class);


    @Override
    public String set(final String bizkey,final String nameSpace,final String value,final int time,final int dbIndex) {
        final String key = getKeyByNamespace(bizkey,nameSpace);
        // 避免setex和setnx问题
        String res = this.performFunction(key,(jedis) -> {
            // set
            jedis.select(dbIndex);
            String tmpres = jedis.set(key,value);
    ...
    @Override
    public String set(final String bizkey,final String nameSpace,final String value,final String nxxx,final String expx,final long time,final int dbIndex) {
        final String key = getKeyByNamespace(bizkey,nameSpace);
        return this.performFunction(key,(jedis) -> {
            jedis.select(dbIndex);
    ...
    @Override
    public String hget(final String bizkey,final String nameSpace,final String field,final GetDataCallBack<String> gbs,final int dbIndex) {
        final String key = getKeyByNamespace(bizkey,nameSpace);
        return this.performFunction(key,(jedis) -> {
            jedis.select(dbIndex);
    ...
    @Override
    public Set<String> zrangeByScore(final String bizkey,final String nameSpace,final double min,final double max,final int offset,final int count,final int dbIndex) {
        final String key = getKeyByNamespace(bizkey,nameSpace);
        return this.performFunction(key,(jedis) -> {
            jedis.select(dbIndex);
    ...
    /**
     * zrevrangeByScore
     * @param bizkey specified key
     * @param max max value
     * @param min min value
     * @param offset offset value
     * @param count count value
     * returnMulti bulk reply specifically a list of elements in the specified
     * range.
     */
    @Override
    public Set<String> zrevrangeByScore(final String bizkey,final String nameSpace,final double max,final double min,final int offset,final int count,final int dbIndex) {
        final String key = getKeyByNamespace(bizkey,nameSpace);
        return this.performFunction(key,(jedis) -> {
            jedis.select(dbIndex);
    ...
    @Override
    public Set<Tuple> zrangeByScoreWithScores(final String bizkey,final String nameSpace,final double min,final double max,final int dbIndex) {
        final String key = getKeyByNamespace(bizkey,nameSpace);
        return this.performFunction(key,(jedis) -> {
            jedis.select(dbIndex);
    ...
    @Override
    public Set<Tuple> zrevrangeByScoreWithScores(final String bizkey,final String nameSpace,final double max,final double min,final int dbIndex) {
        final String key = getKeyByNamespace(bizkey,nameSpace);
        return this.performFunction(key,(jedis) -> {
            jedis.select(dbIndex);
    ...
    @Override
    public Set<Tuple> zrangeByScoreWithScores(final String bizkey,final String nameSpace,final double min,final double max,final int offset,final int count,final int dbIndex) {
        final String key = getKeyByNamespace(bizkey,nameSpace);
        return this.performFunction(key,(jedis) -> {
            jedis.select(dbIndex);
    ...
    @Override
    public Set<Tuple> zrevrangeByScoreWithScores(final String bizkey,final String nameSpace,final double max,final double min,final int offset,final int count,final int dbIndex) {
        final String key = getKeyByNamespace(bizkey,nameSpace);
        return this.performFunction(key,(jedis) -> {
            jedis.select(dbIndex);
    ...
    @Override
    public Long linsert(final String bizkey,final String nameSpace,final BinaryClient.LIST_POSITION where,final String pivot,final String value,final int dbIndex) {
        final String key = getKeyByNamespace(bizkey,nameSpace);
        return this.performFunction(key,(jedis) -> {
            jedis.select(dbIndex);
    ...
    public String echo(final String string,final int dbIndex) {
        return this.performFunction("",(jedis) -> {
            jedis.select(dbIndex);
    ...
    public <T> String set(final String bizkey,final String nameSpace,final T value,final int time,final int dbIndex) {
        final String key = getKeyByNamespace(bizkey,nameSpace);
        return set(bizkey,nameSpace,toJSONString(key,value),time,dbIndex);
    }

    @Override
    public <T> String setex(final String bizkey,final String nameSpace,final int time,final T value,final int dbIndex) {
        final String key = getKeyByNamespace(bizkey,nameSpace);
        return setex(bizkey,nameSpace,time,toJSONString(key,value),dbIndex);
    }
    ...
    public String mset(final Map<String,String> bizkeyValues,final String nameSpace,final int dbIndex) {
        return this.performFunction("",(jedis) -> {
            jedis.select(dbIndex);
            return jedis.mset(smapToArray(bizkeyValues,nameSpace));
        });
    }

    @Override
    public List<String> mget(String[] bizkeys,String nameSpace,final int dbIndex) {
        final String[] key = getKeyByNamespace(bizkeys,nameSpace);
        return this.performFunction("",(jedis) -> {
            jedis.select(dbIndex);
    ...
    public <T> Long hsetObject(String bizkey,String nameSpace,String field,T value,final int dbIndex) {
        final String key = getKeyByNamespace(bizkey,nameSpace);
        return hset(bizkey,nameSpace,field,toJSONString(key,value),dbIndex);
    }

    @Override
    public <T> T hgetObject(final String bizkey,final String nameSpace,final String field,Class<T> value,final GetDataCallBack<T> gbs,final int dbIndex) {
        final String key = getKeyByNamespace(bizkey,nameSpace);
    ...
    @Override
    public <T> T hgetObject(final String bizkey,final String nameSpace,final String field,TypeReference<T> type,final GetDataCallBack<T> gbs,final int dbIndex) {
        final String key = getKeyByNamespace(bizkey,nameSpace);
        String res = hget(bizkey,nameSpace,field,null,dbIndex);
        T rtn = null;
        if (StringUtils.isNotEmpty(res)) {
    ...
    @Override
    public <T> Map<String,T> hgetAllObjects(final String bizkey,final String nameSpace,final TypeReference<T> type,final GetDataCallBack<T> gbs,final int dbIndex) {
        final String key = getKeyByNamespace(bizkey,nameSpace);
        return this.performFunction(key,(jedis) -> {
            jedis.select(dbIndex);
            try {
                ...
                for (Map.Entry<String,String> item : all.entrySet()) {
                    ...
                    T _value = parseObject(key,item.getValue(),type);
    ...
    @Override
    public <T> Map<String,T> hgetAllObjects(final String bizkey,final String nameSpace,final Class<T> value,final GetDataCallBack<T> gbs,final int dbIndex) {
        final String key = getKeyByNamespace(bizkey,nameSpace);
        return this.performFunction(key,(jedis) -> {
            try {
                jedis.select(dbIndex);
                ...
                for (Map.Entry<String,String> item : all.entrySet()) {
                    String _key = item.getKey();
                    T _value = parseObject(key,item.getValue(),value);
                    allObjs.put(_key,_value);
                }
                return allObjs;
            } catch (Exception e) {
    ...
    public Long del(String[] bizkeys,String nameSpace,final int dbIndex) {
        final String[] keys = getKeyByNamespace(bizkeys,nameSpace);
        return this.performFunction("",(jedis) -> {
            jedis.select(dbIndex);
    ...
    public Set<String> sscan(final String bizkey,final String nameSpace,final String match,final int dbIndex) {
        final String key = getKeyByNamespace(bizkey,nameSpace);
        final String fmatch = getKeyByNamespace(match,nameSpace);
        return this.performFunction(key,(jedis) -> {
            try {
                jedis.select(dbIndex);
                int cursor = 0;
    ...
    public Set<String> scan(final String nameSpace,final String match,final int dbIndex) {
        final String fmatch = getKeyByNamespace(match,nameSpace);
        return this.performFunction(null,(jedis) -> {
            try {
                jedis.select(dbIndex);
                int cursor = 0;
                ScanParams scanParams = new ScanParams();
    ...
    public Object eval(final String script,final List<String> bizkeys,final String nameSpace,final List<String> args,final int dbIndex) {
        final List<String> keys = getKeyByNamespace(bizkeys,nameSpace);
        return this.performFunction("",(jedis) -> {
            jedis.select(dbIndex);
    ...
    public Object eval(final String script,final int keyCount,final List<String> params,final String nameSpace,final int dbIndex) {
        final String[] c_params = getKeyArrByNamespace(params,nameSpace);
        return this.performFunction("",(jedis) -> {
            jedis.select(dbIndex);
    ...
    public String rename(final String oldKey,final String newKey,final String nameSpace,final int dbIndex) {
        final String patternOldKey = getKeyByNamespace(oldKey,nameSpace);
        final String patternNewKey = getKeyByNamespace(newKey,nameSpace);
        return this.performFunction("",(jedis) -> {
            jedis.select(dbIndex);
    ...
    public <K,V> Map<K,V> pipelineWithResult(int dbIndex,PipelineCallBackWithResult<K,V> pipelineCallBack) {
        ...
        try {
            ...
            pipeline.sync();
            for (Map.Entry<K,Response<V>> entry : map.entrySet()) {
